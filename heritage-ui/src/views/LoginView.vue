...
const handleLogin = () => {
  ...
  loginRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        const res = await axios.post('http://116.62.165.182/api/login', loginForm.value)
        if (res.data.success) {
...
const handleRegister = () => {
  ...
  regRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        const res = await axios.post('http://116.62.165.182/api/users/register', regForm.value)
        if (res.data.success) {
...
const sendCode = async () => {
  ...
  try {
    const res = await axios.post(`http://116.62.165.182/api/users/forgot-password?email=${forgotDialog.value.email}`)
    if (res.data.success) {
      ElMessage.success(res.data.message)
...
const resetPwd = async () => {
  ...
  try {
    const res = await axios.post(`http://116.62.165.182/api/users/reset-password?email=${forgotDialog.value.email}&code=${forgotDialog.value.code}&newPassword=${forgotDialog.value.newPassword}`)
    if (res.data.success) {
